(() => {
  const App = {
    htmlElements: {
      ...
      closeModalAgregarManga: document.getElementById('close-modal-agregar-manga'),
      closeModalEditarManga: document.getElementById('close-modal-manga'),
      closeModalAgregarCapitulo: document.getElementById('close-modal-agregar-capitulo'),
      ...
      cancelarAgregarCapitulo: document.getElementById('cancelar-agregar-capitulo'),
      cancelarEditarCapitulo: document.getElementById('cancelar-editar-capitulo'),
    ...
    mangas: [
      {
        ...
        genero: "accion",
        sinopsis: "Goku y sus amigos protegen la Tierra de amenazas alienígenas",
        portada: "https://tse1.mm.bing.net/th/id/OIP.qv89cUB2sU6r26vAbYv_vwHaLu?r=0&rs=1&pid=ImgDetMain&o=7&rm=3",
        capitulos: [
          { id: 1, numero: 1, nombre: "El Regreso de Goku", precio: 15, sinopsis: "Goku regresa de Namek" },
          { id: 2, numero: 2, nombre: "La Llegada de Vegeta", precio: 10, sinopsis: "Vegeta llega a la Tierra" },
          { id: 3, numero: 3, nombre: "La Batalla Final", precio: 20, sinopsis: "La batalla decisiva" }
        ]
      },
      {
        id: 2,
        ...
        portada: "https://ramenparados.com/wp-content/uploads/2015/09/portada_naruto-n-70_masashi-kishimoto_2015082513321.jpg",
        capitulos: [
          { id: 1, numero: 1, nombre: "El Inicio del Viaje", precio: 50, sinopsis: "Naruto comienza su entrenamiento" },
          { id: 2, numero: 2, nombre: "El Examen Chunin", precio: 20, sinopsis: "Comienza el examen" }
        ]
      },
      {
        id: 3,
        ...
        portada: "https://www.anmosugoi.com/wp-content/uploads/2022/07/One-Piece-manga-vol-103-1200x1883.jpg",
        capitulos: [
          { id: 1, numero: 1, nombre: "El Sombrero de Paja", precio: 10, sinopsis: "Luffy obtiene su sombrero" },
          { id: 2, numero: 2, nombre: "La Búsqueda del Tesoro", precio: 15, sinopsis: "Comienza la aventura" }
        ]
      }
    ],
    ...
    bindEvents() {
      ...
      if (this.htmlElements.btnMangaView) {
        this.htmlElements.btnMangaView.addEventListener('click', () => this.mostrarVista('manga'));
      }
      if (this.htmlElements.btnCapitulosView) {
        this.htmlElements.btnCapitulosView.addEventListener('click', () => this.mostrarVista('capitulos'));
      }
      
      // Botones de agregar
      if (this.htmlElements.btnAgregarManga) {
        this.htmlElements.btnAgregarManga.addEventListener('click', () => this.abrirModal('agregar-manga'));
      }
      if (this.htmlElements.btnAgregarCapitulo) {
        this.htmlElements.btnAgregarCapitulo.addEventListener('click', () => this.abrirModal('agregar-capitulo'));
      }
      
      // Cerrar modales
      if (this.htmlElements.closeModalAgregarManga) {
        this.htmlElements.closeModalAgregarManga.addEventListener('click', () => this.cerrarModal('agregar-manga'));
      }
      if (this.htmlElements.closeModalEditarManga) {
        this.htmlElements.closeModalEditarManga.addEventListener('click', () => this.cerrarModal('editar-manga'));
      }
      if (this.htmlElements.closeModalAgregarCapitulo) {
        this.htmlElements.closeModalAgregarCapitulo.addEventListener('click', () => this.cerrarModal('agregar-capitulo'));
      }
      if (this.htmlElements.closeModalEditarCapitulo) {
        this.htmlElements.closeModalEditarCapitulo.addEventListener('click', () => this.cerrarModal('editar-capitulo'));
      }
      
      // Cancelar modales
      if (this.htmlElements.cancelarAgregarManga) {
        this.htmlElements.cancelarAgregarManga.addEventListener('click', () => this.cerrarModal('agregar-manga'));
      }
      if (this.htmlElements.cancelarEditarManga) {
        this.htmlElements.cancelarEditarManga.addEventListener('click', () => this.cerrarModal('editar-manga'));
      }
      if (this.htmlElements.cancelarAgregarCapitulo) {
        this.htmlElements.cancelarAgregarCapitulo.addEventListener('click', () => this.cerrarModal('agregar-capitulo'));
      }
      if (this.htmlElements.cancelarEditarCapitulo) {
        this.htmlElements.cancelarEditarCapitulo.addEventListener('click', () => this.cerrarModal('editar-capitulo'));
      }
      
      // Formularios
      if (this.htmlElements.formAgregarManga) {
        this.htmlElements.formAgregarManga.addEventListener('submit', (e) => this.agregarManga(e));
      }
      if (this.htmlElements.formEditarManga) {
        this.htmlElements.formEditarManga.addEventListener('submit', (e) => this.guardarManga(e));
      }
      if (this.htmlElements.formAgregarCapitulo) {
        this.htmlElements.formAgregarCapitulo.addEventListener('submit', (e) => this.agregarCapitulo(e));
      }
      if (this.htmlElements.formEditarCapitulo) {
        this.htmlElements.formEditarCapitulo.addEventListener('submit', (e) => this.guardarCapitulo(e));
      }
    },

    mostrarVista(vista) {
      if (vista === 'manga') {
        if (this.htmlElements.mangaView) this.htmlElements.mangaView.classList.remove('hidden');
        if (this.htmlElements.capitulosView) this.htmlElements.capitulosView.classList.add('hidden');
      } else {
        if (this.htmlElements.mangaView) this.htmlElements.mangaView.classList.add('hidden');
        if (this.htmlElements.capitulosView) this.htmlElements.capitulosView.classList.remove('hidden');
      }
    },
    ...
    agregarCapitulo(e) {
      e.preventDefault();
      const formData = new FormData(e.target);
      const mangaSeleccionado = this.mangas.find(m => m.nombre === this.htmlElements.mangaSeleccionado.textContent);
